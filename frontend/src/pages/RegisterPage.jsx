...
export default function RegisterPage() {
  ...
  const [isOAEnabled, setIsOAEnabled] = useState(false);
    ...
    const checkPhase = async () => {
      try {
        ...
        if (data.success && data.data) {
          setIsOAEnabled(data.data.isOAEnabled);
        }
      } catch (err) {
        console.error(err);
      } finally {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-dark-900 via-dark-800 to-dark-700 px-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-dark-700/80 backdrop-blur-sm border border-dark-500/50 rounded-2xl p-8 shadow-2xl">
          {checkingPhase ? (
            <div className="flex justify-center p-8"><span className="text-gray-400 animate-pulse">Loading...</span></div>
          ) : !isOAEnabled ? (
            <div className="text-center py-6">
              <div className="w-16 h-16 bg-gray-500/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <span className="text-2xl">🔒</span>
              </div>
              <h2 className="text-2xl font-display font-semibold text-white mb-3">Registrations Closed</h2>
              <p className="text-gray-400 text-sm mb-6">
                Online Assessment registrations are not yet open. You must be shortlisted during the application phase to register.
              </p>
              <Link to="/" className="text-brand-400 hover:text-brand-300 font-medium text-sm">
                Return to Home
              </Link>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {[
                  ...
                  { label: 'Email address (from application)', field: 'email', type: 'email', placeholder: 'you@example.com' },
